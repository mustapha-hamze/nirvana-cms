...
export async function attachDetailsToParents(DetailModel, parentField, parents, { application, langKey, status } = {}) {
  const parentIds = parents.map((p) => p._id);
  const filter = { application, [parentField]: { $in: parentIds } };
  ...
  return parents.map((p) => ({
    ...p.toObject(),
    details: byParentId.get(p._id.toString()) ?? [],
  }));
}
