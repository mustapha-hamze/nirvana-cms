...
// "never existed".
export async function resolveFrontendApp(req: Request, res: Response, next: NextFunction) {
  const appKey = req.query.appKey || req.headers["x-app-key"];
  if (!appKey) return res.status(400).json({ message: "appKey is required" });

  const application = await Application.findOne({ appKey: appKey as string, status: "active" });
  ...
  if (lang && !allowedLanguages.includes(lang as string)) {
  ...
  req.langKey = (lang as string) || allowedLanguages[0];
  next();
}
