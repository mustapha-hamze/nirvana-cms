...
const SECTION_LAYOUTS_BY_TYPE: Record<string, (typeof SECTION_LAYOUTS)[keyof typeof SECTION_LAYOUTS]> = SECTION_LAYOUTS;
const PAGE_COMPONENT_LAYOUTS_BY_TYPE: Record<string, (typeof PAGE_COMPONENT_LAYOUTS)[keyof typeof PAGE_COMPONENT_LAYOUTS]> = PAGE_COMPONENT_LAYOUTS;
...
export function validateContentSections(sections: any): ValidationResult {
  ...
  for (let i = 0; i < sections.length; i++) {
    ...
    const layout = SECTION_LAYOUTS_BY_TYPE[section?.type];
    if (!layout) {
      return {
        valid: false,
    ...
    for (const slot of layout.slots) {
      for (let s = 0; s < slot.count; s++) {
        ...
        if (!(slot.elementTypes as readonly string[]).includes(element?.elementType)) {
          return {
            valid: false,
...
export function validatePageSections(sections: any): ValidationResult {
  if (sections === undefined) return { valid: true };
  if (!Array.isArray(sections)) {
    return { valid: false, message: "sections must be an array" };
  }
  ...
  for (let i = 0; i < sections.length; i++) {
    ...
    for (let j = 0; j < components.length; j++) {
      ...
      const layout = PAGE_COMPONENT_LAYOUTS_BY_TYPE[component?.type];
      if (!layout) {
        return {
          valid: false,
      ...
      const wrongType = elements.find((el: any) => el?.elementType !== layout.elementType);
      if (wrongType) {
        return {
          valid: false,
