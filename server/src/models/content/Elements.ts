...
export const urlField = (extra: Record<string, unknown> = {}) => ({
  type: String,
  trim: true,
  default: '',
  validate: {
    validator: (v: string) => !v || URL_PATTERN.test(v),
    message: (props: { value: string }) =>
      `${props.value} must be an absolute http(s) URL, a path starting with "/", or a filename`,
  },
  ...extra,
})
...
export const imageGallerySchema = new mongoose.Schema(
  {
    images: {
      ...
      validate: {
        validator: (arr: unknown[]) => Array.isArray(arr) && arr.length >= 2 && arr.length <= 20,
...
export function attachElementDiscriminators(elementsPath: mongoose.Schema.Types.DocumentArray) {
