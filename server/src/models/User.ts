...
export interface UserDoc extends mongoose.Document {
  ...
  // Populated with `.populate('applications', 'name')` in some code paths
  // (see middleware/auth.js) and left as plain ObjectIds in others — kept
  // loose rather than modeling both shapes.
  applications: any[];
  isDeleted: boolean;
...
userSchema.pre('validate', function enforceApplicationRule() {
  if (!APP_SCOPED_ROLES.includes(this.role as (typeof APP_SCOPED_ROLES)[number])) return
