...
const MAGIC_BYTES_BY_MIME: Record<string, (buf: Buffer) => boolean> = {
...
function buildUploadMiddleware(kind: UploadKind) {
  ...
  return function rawFileUploadMiddleware(req: Request, res: Response, next: NextFunction) {
...
async function saveRawFile(kind: UploadKind, buffer: Buffer, mimetype: string, domain: MediaDomain) {
  const config = KIND_CONFIG[kind]
...
export function saveVideo(buffer: Buffer, mimetype: string, domain: MediaDomain) {
  return saveRawFile('video', buffer, mimetype, domain)
}

export function saveDocument(buffer: Buffer, mimetype: string, domain: MediaDomain) {
