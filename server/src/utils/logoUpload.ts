...
export async function saveApplicationLogo(
  ...
  if (metadata.width !== REQUIRED_SIZE || metadata.height !== REQUIRED_SIZE) {
    throw Object.assign(new Error('Logo must be exactly 1024×1024 pixels'), { status: 400 })
  }
...
export async function deleteApplicationLogo(applicationId: mongoose.Types.ObjectId | string) {
  const dir = path.join(STORAGE_ROOT, applicationId.toString())
