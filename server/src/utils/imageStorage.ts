...
export const IMAGE_MAX_BYTES = 2 * 1024 * 1024 // 2MB
...
// instead of falling through to the generic 500 handler in index.js. Shared
// across domains — validation/size rules don't vary by content vs page.
export function imageUploadMiddleware(req: Request, res: Response, next: NextFunction) {
  upload(req, res, (err: unknown) => {
    if (err instanceof multer.MulterError && err.code === 'LIMIT_FILE_SIZE') {
      return next(Object.assign(new Error('Image must be smaller than 2MB'), { status: 400 }))
    }
...
function encodeForMimetype(pipeline: ReturnType<typeof sharp>, mimetype: string) {
  switch (mimetype) {
    case 'image/jpeg':
...
// uploaded before the content/page item itself has been saved for the first
...
export async function saveImage(buffer: Buffer, mimetype: string, domain: MediaDomain) {
