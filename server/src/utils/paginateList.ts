export const SORT_BY_VALUES = Object.freeze(["title", "createdAt"] as const);
export const SORT_ORDER_VALUES = Object.freeze(["asc", "desc"] as const);
...
export function normalizePaging(page: unknown, limit: unknown) {
...
interface PaginateListOptions<T> {
  idOf: (item: T) => string;
  titleOf: (item: T) => string;
  createdAtOf: (item: T) => Date | string;
  search?: string | null;
  sortBy?: SortBy | null;
...
export function paginateList<T>(
  items: T[],
  { idOf, titleOf, createdAtOf, search, sortBy, sortOrder, page, limit }: PaginateListOptions<T>,
) {
