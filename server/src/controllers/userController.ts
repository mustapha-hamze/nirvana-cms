...
function buildEmail(role: string, applications: unknown[], email: string) {
  if (role === ROLES.WEBSITE_USER) return `${applications[0]}-${email}`
  return email
}

function parseDisplayEmail(user: any) {
...
async function validateApplicationIds(ids: unknown[]) {
  ...
  const found = await Application.countDocuments({ _id: { $in: ids as string[] } })
...
export async function createUser(req: Request, res: Response) {
  ...
  const appIds: unknown[] = Array.isArray(applications) ? applications : applications ? [applications] : []

  if ((APP_SCOPED_ROLES as string[]).includes(role) && appIds.length === 0) {
...
export async function updateUserStatus(req: Request, res: Response) {
  ...
  const allowedStatuses = User.schema.path('status').enumValues as string[]
...
export async function unassignApplication(req: Request, res: Response) {
  const { appId } = req.params

  const user = await User.findById(req.params.id)
  if (!user) return res.status(404).json({ message: 'User not found' })

  if (user.applications.length <= 1 && (APP_SCOPED_ROLES as string[]).includes(user.role)) {
